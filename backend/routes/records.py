...
async def get_records(wallet: str, limit: int = 100):
    ...
    try:
        ...
        logger.info(f"[GET RECORDS] Fetching records for wallet: {wallet} (limit: {limit})")
        ...
        logger.info(f"[GET RECORDS] Database returned {len(records)} records")
        
        # SECURITY: Remove encryption keys from response - they should never leave the client
        for record in records:
            # Remove encryption key if present (security)
            if "encryption_key" in record:
                del record["encryption_key"]
        ...
        logger.info(f"[GET RECORDS] Returning {len(records)} records to frontend")
...
async def update_record(record_id: str, body: UpdateRecordRequest):
    ...
    # SECURITY: Never accept or store encryption keys from client
    if "encryption_key" in payload:
        del payload["encryption_key"]
    ...
    try:
        ...
        # SECURITY: Remove encryption key from response
        if updated and "encryption_key" in updated:
            del updated["encryption_key"]
...
async def check_cache(body: CloneRecordRequest):
    ...
    try:
        ...
        # SECURITY: Remove encryption key from response
        if existing and "encryption_key" in existing:
            del existing["encryption_key"]
...
async def clone_record(body: Dict[str, Any]):
    ...
    try:
        ...
        # SECURITY: Remove encryption key from response
        if new_record and "encryption_key" in new_record:
            del new_record["encryption_key"]
