...
export default function DoctorDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'patients' | 'appointments' | 'profile' | 'ai-assistant'>('patients');
    ...
    // Get wallet address from unified auth (userId is either Clerk ID or wallet address)
    const address = userId || undefined;
    ...
    useEffect(() => {
        ...
        if (!isAuthenticated) {
            console.log('[Doctor Dashboard] Not authenticated, redirecting to home');
            router.push('/');
            return;
        }
        ...
        if (userRole && userRole !== 'doctor') {
            console.log('[Doctor Dashboard] User is not a doctor, redirecting');
            router.push('/patient');
            return;
        }

        console.log('[Doctor Dashboard] User authenticated, loading data');
        loadProfile();
        loadGrantedRecords();
        loadAppointments();
    }, [isAuthenticated, userId, authMethod, router]);
    ...
    const saveProfile = async () => {
        if (!address) {
            alert('Wallet address not found. Please ensure you are logged in.');
            return;
        }

        if (!profileForm.name || !profileForm.specialty) {
            alert('Please fill in all required fields (Name and Specialty).');
            return;
        }
    ...
    return (
        <div className="min-h-screen pt-24 pb-12 px-6 relative bg-white">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="flex gap-2 overflow-x-auto pb-6 border-b border-slate-300 mb-8">
                    {(['patients', 'ai-assistant', 'appointments', 'profile'] as const).map(tab => (
                        <button
                            key={tab}
                        ...
                        >
                            {tab === 'patients' ? 'Patient Records' : 
                             ...
                             tab === 'appointments' ? 'Schedule & Appts' : 'Provider Profile'}
                        </button>
                    ))}
                </div>
                                        ...
                                        >
                                            <div className="flex justify-between items-start gap-2 mb-1.5">
                                                <div className="flex-1">
                                                    {g.patient_profile?.full_name ? (
                                                        <>
                                                            <h3 className={`font-bold text-base mb-0.5 ${selectedGrant?.id === g.id ? 'text-blue-900' : 'text-gray-900'}`}>
                                                                {g.patient_profile.full_name}
                                                            </h3>
                                                            <p className="text-xs text-gray-500">
                                                                {g.patient_profile.gender && `${g.patient_profile.gender.charAt(0).toUpperCase() + g.patient_profile.gender.slice(1)}`}
                                                                {g.patient_profile.gender && g.patient_profile.blood_type && ' • '}
                                                                {g.patient_profile.blood_type && `${g.patient_profile.blood_type}`}
                                                            </p>
                                                        </>
                                                    ) : (
                                                        <h3 className={`font-semibold text-sm ${selectedGrant?.id === g.id ? 'text-blue-900' : 'text-gray-900'}`}>
                                                            Patient {g.patient_wallet?.slice(0, 6)}
                                                        </h3>
                                                    )}
                                                    <p className={`font-medium text-xs mt-1 truncate ${selectedGrant?.id === g.id ? 'text-blue-700' : 'text-gray-600'}`}>
                                            ...
                                            <div className="flex items-center justify-between text-[10px] text-gray-500 font-medium mt-2">
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        router.push(`/doctor/patient/${g.patient_wallet}`);
                                                    }}
                        ...
                        <div className="h-full">
                            {selectedGrant?.analysis ? (
                                <div className="space-y-6 animate-fade-in">
                                    {/* Patient Info Banner */}
                                    {selectedGrant.patient_profile?.full_name && (
                                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 border border-blue-100">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-lg">
                                                    {selectedGrant.patient_profile.full_name.charAt(0).toUpperCase()}
                                                </div>
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-3 text-sm text-gray-600">
                                                        {selectedGrant.patient_profile.gender && (
                                                            <span>{selectedGrant.patient_profile.gender.charAt(0).toUpperCase() + selectedGrant.patient_profile.gender.slice(1)}</span>
                                                        )}
                                                        {selectedGrant.patient_profile.blood_type && (
                                                            <>
                                                                <span>•</span>
                                                                <span>Blood Type: {selectedGrant.patient_profile.blood_type}</span>
                                                            </>
                                                        )}
                                                        {selectedGrant.patient_profile.date_of_birth && (
                                                            <>
                                                                <span>•</span>
                                                                <span>DOB: {new Date(selectedGrant.patient_profile.date_of_birth).toLocaleDateString()}</span>
                                                            </>
                                                        )}
                                                    </div>
                                                </div>
                                                <button
                                                    onClick={() => router.push(`/doctor/patient/${selectedGrant.patient_wallet}`)}
                ...
                {activeTab === 'ai-assistant' && (
                    <div className="animate-fade-in space-y-6">
                        <div className="mb-6">
                            <h2 className="text-2xl font-semibold text-gray-900 mb-2">AI Medical Assistant</h2>
                            <p className="text-sm text-gray-600">
                                Chat with AI to analyze patient records, get treatment suggestions, and draft clinical notes.
                            </p>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                ...
                                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-5 border border-blue-200">
                                    <div className="flex items-center gap-3 mb-3">
                                        <div className="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold">
                                            {selectedGrant.patient_wallet.slice(0, 2).toUpperCase()}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h3 className="text-sm font-semibold text-gray-900">Patient ID</h3>
                                            <p className="text-xs font-mono text-gray-600 truncate">
                                                {selectedGrant.patient_wallet.slice(0, 10)}...{selectedGrant.patient_wallet.slice(-6)}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="space-y-2 text-xs">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-semibold text-gray-900 truncate ml-2">{selectedGrant.analysis.file_name}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Uploaded:</span>
                                            <span className="font-semibold text-gray-900">{new Date(selectedGrant.analysis.created_at).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-2xl p-5 border border-orange-200">
                                    <div className="flex items-center justify-between mb-3">
                                        ...
                                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                                            selectedGrant.analysis.urgency === 'critical' ? 'bg-red-100 text-red-700' :
                                            selectedGrant.analysis.urgency === 'high' ? 'bg-orange-100 text-orange-700' :
                                            selectedGrant.analysis.urgency === 'medium' ? 'bg-yellow-100 text-yellow-700' :
                                    ...
                                    <div className="flex items-baseline gap-2 mb-2">
                                        <span className="text-4xl font-bold text-gray-900">{selectedGrant.analysis.risk_score}</span>
                                        <span className="text-sm text-gray-500">/100</span>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-2">
                                        <div 
                                            className={`h-2 rounded-full ${
                                                selectedGrant.analysis.risk_score >= 70 ? 'bg-red-500' :
                                                selectedGrant.analysis.risk_score >= 50 ? 'bg-orange-500' :
                                                selectedGrant.analysis.risk_score >= 30 ? 'bg-yellow-500' :
                                                'bg-green-500'
                                            }`}
                                            style={{ width: `${selectedGrant.analysis.risk_score}%` }}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-5 border border-purple-200">
                                    ...
                                    <div className="space-y-2">
                                        {selectedGrant.analysis.conditions && selectedGrant.analysis.conditions.length > 0 ? (
                                            selectedGrant.analysis.conditions.slice(0, 3).map((condition, idx) => (
                                                <div key={idx} className="flex items-center gap-2">
                                                    <div className="w-1.5 h-1.5 rounded-full bg-purple-500 shrink-0" />
                                                    <span className="text-xs text-gray-700 truncate">{condition}</span>
                                                </div>
                                            ))
                                        ) : (
                                            <p className="text-xs text-gray-500 italic">No specific conditions detected</p>
                                        )}
                                        {selectedGrant.analysis.conditions && selectedGrant.analysis.conditions.length > 3 && (
                                            <p className="text-xs text-purple-600 font-semibold">
                                                +{selectedGrant.analysis.conditions.length - 3} more
                                            </p>
                                        )}
                                    </div>
                                    <div className="mt-3 pt-3 border-t border-purple-200">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-semibold text-gray-700">{selectedGrant.analysis.specialist}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="h-[600px]">
                            <DoctorAIChatbot 
                                patientWallet={selectedGrant?.patient_wallet}
                                analysisData={selectedGrant?.analysis}
                            />
                        </div>
                        ...
                        {!selectedGrant && (
                            <div className="mt-4 bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-semibold text-blue-900 mb-1">Tip: Select a patient record</p>
                                    <p className="text-xs text-blue-700">
                                        Go to the "Patient Records" tab and select a patient's analysis to get context-aware AI assistance.
                                    </p>
                                </div>
                            </div>
                        )}
