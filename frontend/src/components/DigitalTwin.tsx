...
export default function DigitalTwin({ analysisData, recordId }: DigitalTwinProps) {
    ...
    useEffect(() => {
        ...
        // Use actual risk score from analysis if available
        const actualRiskScore = analysisData.risk_score || 0;
        const actualConditions = analysisData.conditions || [];
        const actualSummary = (analysisData.summary || '').toLowerCase();
        
        // Determine overall status based on actual risk score
        ...
        const overallStatus = getStatusFromScore(actualRiskScore);

        // Map conditions to specific organs
        const organConditions: Record<string, string[]> = {
            'Brain': [],
            'Lungs': [],
            'Heart': [],
            'Liver': [],
        ...
        // Update each organ based on actual conditions
        const createOrganStatus = (organName: string, conditions: string[]): Omit<OrganStatus, 'muscleId'> => {
            const base = { 
                aiInsights: [] as string[], 
                recommendations: [] as string[], 
                markers: [] as { name: string; value: string; status: Status }[] 
            };
        ...
        setSystemStatus(prev => ({
            'Brain': { ...prev['Brain'], ...createOrganStatus('Brain', organConditions['Brain']), muscleId: 'BRAIN' },
            'Lungs': { ...prev['Lungs'], ...createOrganStatus('Lungs', organConditions['Lungs']), muscleId: 'LUNGS' },
            'Heart': { ...prev['Heart'], ...createOrganStatus('Heart', organConditions['Heart']), muscleId: 'HEART' },
            'Liver': { ...prev['Liver'], ...createOrganStatus('Liver', organConditions['Liver']), muscleId: 'LIVER' },
            'Gut': { ...prev['Gut'], ...createOrganStatus('Gut', organConditions['Gut']), muscleId: 'GUT' },
            'Kidneys': { ...prev['Kidneys'], ...createOrganStatus('Kidneys', organConditions['Kidneys']), muscleId: 'KIDNEYS' },
        }));
    }, [analysisData]);
